...
class SearchThread(QThread):
    ...
    def __init__(self, query: str, limit: int = 100, year_min=None, year_max=None, songs_min=None, songs_max=None):
        super().__init__()
        self.query = query
        self.limit = limit
...
def run_docker_search(query: str, limit: int = 50, year_min=None, year_max=None, songs_min=None, songs_max=None) -> list[dict]:
    """
    Spustí vyhľadávanie V DOKERI pomocou:
    docker exec <container> python /app/search.py --index-dir ... --query ... --limit ...
    a vráti list dictov s výsledkami.
    """
    cmd = [
        "docker",
        #"compose",
    ...
    # spustíme docker exec a chytíme stdout
    proc = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8')

    if proc.returncode != 0:
        # pre debug si vypíš chybu
...
class MainWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        # horný riadok: label + text input + Search button
        top_row = QHBoxLayout()
        ...
        self.table.setHorizontalHeaderLabels(
            ["Artist", "Venue", "City", "Country", "Date", "Tour", "Songs", "Score", "View"]
        )
    ...
    def run_search(self):
        ...
        # Get filter values
        year_min = self.year_min_spin.value() if self.year_min_check.isChecked() else None
        year_max = self.year_max_spin.value() if self.year_max_check.isChecked() else None
        songs_min = self.songs_min_spin.value() if self.songs_min_check.isChecked() else None
        songs_max = self.songs_max_spin.value() if self.songs_max_check.isChecked() else None
    ...
    def _populate_table(self, results: list[dict]):
        ...
        for row, r in enumerate(results):
            ...
            self.table.setItem(row, 6, QTableWidgetItem(str(r.get("songs_count") or "")))
...
"""
        ...
        # Venue info
        if doc.get('venue_bio') or doc.get('venue_capacity') or doc.get('venue_location') or doc.get('venue_opened'):
        ...
        # Country info
        if doc.get('country_bio') or doc.get('country_capital') or doc.get('country_area') or doc.get('country_population'):
            ...
            if doc.get('country_population'):
                content += f"<p><b>Population:</b> {doc.get('country_population')}</p>\n"
